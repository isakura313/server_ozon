...
exports.delete = (req, res) => {
    Deal.remove(req.params.id, (err, data) => {
        if (err) {
            if (err.kind === "not_found") {
                res.status(404).send({
                    message: `Не найдено дело с ID ${req.params.id}.`
                });
            } else {
                res.status(500).send({
                    message: " не могу удалить дело с ID " + req.params.id
                });
            }
        } else res.send({message: `deal было удалено успешно!`});
    });
};
...
exports.deleteAll = (req, res) => {
    Deal.removeAll((err, data) => {
        ...
        else res.send({message: `All Deals were deleted successfully!`});
    });
};
